    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        when(item.itemId){
            ...
            R.id.ic_phoneBook -> {
                ...
                supportFragmentManager.beginTransaction().replace(R.id.fragment_frame,phoneBookFragment).commit()
            }
            R.id.ic_recentCall -> {
                Log.d(TAG,"Menubar - 최근기록 클릭")
                recentCallFragment =RecentCallFragment.newInstance()
                supportFragmentManager.beginTransaction().replace(R.id.fragment_frame,recentCallFragment).commit()
            }
            R.id.ic_setting -> {
                Log.d(TAG,"Menubar - 설정 클릭")
                settingFragment =SettingFragment.newInstance()
                supportFragmentManager.beginTransaction().replace(R.id.fragment_frame,settingFragment).commit()
            }
        }
